...
This code was changed (only a small amount so far) by Benjamin Kovacs
This code is still being worked on and will probably not be useful to anyone yet
If you would like to use Open CV for  vision processing for First Robotics Competition
please see the original code at https://github.com/Team2168/2168_Vision_Example
	...
	while (true)
	{
		...
		if (params.Process && progRun)
		{
			...
			if (!frame.empty())
			{
				...
				///////TODO: put above values into network tables////////////
 ...
 * in its own thread so it can run as fast as possible and store frames.
 *
